...
- [ ] TACTICAL TWIN STAGE 2: Production-verify the new connector-backed Generate Cinematic Replay flow end to end with the user-approved 32.5-credit run: submit from the actual UI, handle paid confirmation, poll to completion, retain automatically, preview/download, test cancellation semantics, create a secure share, and revoke it without manual import
- [ ] BUG: Replace the cinematic panel’s silent browser video-frame capture dependency with owner-authorized server-assisted FFmpeg keyframe extraction so the actual Generate button always creates a connector-backed export from uploaded film
- [ ] BUG: Recover a completed connector task that returns a Higgsfield provider job ID but omits the MP4 URL by launching one non-generative status-recovery task and retaining the existing paid output without submitting or charging a duplicate generation
- [ ] TACTICAL TWIN STAGE 2: Production-verify private preview, public share playback, and revocation from the actual UI; then close final ownership, token, retry, cleanup, accessibility, desktop/mobile, tests, type-check, and build validation
