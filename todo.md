...
- [x] BUG: Fix Live session 2310001 five-second `analyzeWindow` failure so malformed, empty, timed-out, or provider-rejected AI responses persist a recoverable evidence event, keep exactly two predictions, and allow subsequent windows to continue without surfacing a mutation error
- [x] VALIDATION: Production session 2310001 window 10 returned `recovered: true` in 2.8 seconds without a mutation exception; persisted 1 captured frame at 0% confidence with exactly two 50/50 predictions; rendered the nonfatal recovery banner and 0:50–0:55 evidence card; and left learned game memory at the prior 3 genuine windows
...
- [x] TACTICAL TWIN STAGE 2: Implemented automatic football-player and ball tracking from bounded server-extracted 960×540 source frames with confidence, anonymous stable IDs, occlusion state, resumable one-frame model batches, and exact timestamps; real Twin 30001 extraction persisted 24 anonymous detections per frame
...
- [x] BUG: Replaced production Auto Track's unreliable hidden-video `loadedmetadata` dependency with bounded server-assisted exact-frame extraction; Twin 30001 completed 24/24 real frames and entered coach review
- [x] BUG: Added FFmpeg to the production runtime, verified bounded frame extraction in the deployed app, and completed the full real Twin 30001 tracking job without `spawn ffmpeg ENOENT`
- [x] BUG: Added percentage confidence guardrails and an audit-preserving Reprocess Confidence path; production Twin 30001 completed a new 24/24 review set at 85–95% frame confidence and 75% field calibration while retaining the original legacy evidence
- [x] TACTICAL TWIN STAGE 2: Built and deployed the app-scoped Manus/Higgsfield connector bridge; a live private non-generative task confirmed image-to-video availability, and Generate now submits, confirms, polls, cancels, and retains connector-backed exports without manual import
- [x] TACTICAL TWIN STAGE 2: Production-verified the connector-backed Generate Cinematic Replay flow with the user-approved 32.5-credit run: the actual UI created export 810001, submitted one paid provider job, recovered its output without duplicate generation, retained it automatically, exposed private preview/download, created a secure share, and revoked every test link
...
- [x] VALIDATION: Reopened export 810001 in the authenticated Tactical Twin gallery and captured the real football frame visibly rendered inside its native private replay player, with Preview, signed Download, Share, retained poster, controls, muted autoplay, loop, and five-second playback asset intact
- [x] TACTICAL TWIN STAGE 2: Production-verified corrected 24/24 tracking, 0.25× and 33 ms frame-step controls, automatic connector export, actual gallery cancellation, private retained playback, public safe projection, download, immediate revocation, ownership/privacy boundaries, desktop/mobile responsiveness, accessibility, retry/recovery, cleanup, 32/32 test files, 142/142 tests, strict type-check, and production build
