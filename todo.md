...
- [ ] BUG: Add model-confidence normalization guardrails, deploy the explicit 0–100 contract, and verify a fresh real tracking frame no longer ambiguously treats unit-scale confidence as literal percentages
- [ ] TACTICAL TWIN STAGE 2: Build and enable a connector-backed scheduled export bridge so Generate Cinematic Replay queues a request that the authenticated Higgsfield connector completes and retains without a manual one-off import
