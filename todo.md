...
- [ ] HONESTY GAPS (acknowledged, see below): progress stages are timer-simulated (not backend-driven), spotlight/ask-film circles are AI-estimated positions (not frame-level tracking), clip timestamps inherit the estimated-timestamp limitation
- [ ] Implement real backend progress reporting (download/frames/LLM/report stages persisted to DB, polled by AnalysisProgress) instead of timer-based simulation
- [ ] Label Player Spotlight and Ask-the-Film circle overlays as "AI-estimated" in the UI until true frame-level player localization exists
