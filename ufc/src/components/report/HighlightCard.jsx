...
const categoryStyles = {
  striking: 'bg-orange-500/10 text-orange-400',
  grappling: 'bg-blue-500/10 text-blue-400',
  clinch: 'bg-amber-400/10 text-amber-400',
  mistake: 'bg-purple-500/10 text-purple-400',
};

const verdictStyles = {
  good: { border: 'border-l-[#FF2D2D]', text: 'text-[#FF2D2D]', icon: CheckCircle2, label: 'Landed clean' },
  bad: { border: 'border-l-red-500', text: 'text-red-400', icon: XCircle, label: 'Got caught' },
};
    ...
    <div className={`bg-[#161B22] border border-white/5 rounded-xl p-4 ${verdict ? `border-l-2 ${verdict.border}` : ''}`}>
      <div className="flex items-center gap-2 mb-2 flex-wrap">
        <button
          ...
          className="inline-flex items-center gap-1.5 text-xs font-mono font-semibold text-[#FF2D2D] bg-[#FF2D2D]/10 px-2 py-1 rounded-md hover:bg-[#FF2D2D]/20 transition-colors"
        >
          <Play className="w-3 h-3" />
          {highlight.timestamp}
        </button>
        <span className={`text-[10px] font-semibold uppercase tracking-wider px-2 py-1 rounded-full ${categoryStyles[highlight.category] || categoryStyles.striking}`}>
          {highlight.category}
        </span>
        {verdict && (
      ...
      <button
        ...
        className="mt-3 inline-flex items-center gap-1.5 text-xs font-semibold text-slate-400 hover:text-[#FF2D2D] transition-colors"
      >
        {showClip ? <ChevronUp className="w-3.5 h-3.5" /> : <Video className="w-3.5 h-3.5" />}
        {showClip ? 'Hide clip' : 'Watch this exchange'}
      </button>
