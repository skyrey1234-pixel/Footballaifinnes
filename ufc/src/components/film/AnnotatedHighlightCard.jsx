...
import { Play, Eye, EyeOff, Sparkles, AlertTriangle, CheckCircle2, Zap, ChevronDown, ChevronUp, RotateCcw } from 'lucide-react';
import AnnotationCanvas from './AnnotationCanvas';
import { generateHighlightAnnotations } from '@/lib/fightAnnotation';

const CATEGORY_STYLES = {
  striking: { bg: 'bg-orange-500/10', text: 'text-orange-400', border: 'border-orange-500/30' },
  grappling: { bg: 'bg-blue-500/10', text: 'text-blue-400', border: 'border-blue-500/30' },
  clinch: { bg: 'bg-amber-400/10', text: 'text-amber-400', border: 'border-amber-400/30' },
...
const VERDICT_STYLES = {
  ...
  good: {
    ...
    icon: CheckCircle2,
    label: 'Clean',
    headerBg: 'from-[#22C55E]/10 to-transparent',
  },
};
...
export default function AnnotatedHighlightCard({ highlight, session, report, index }) {
  ...
  const catStyle = CATEGORY_STYLES[highlight.category] || CATEGORY_STYLES.striking;
    ...
    <div className={`bg-[#161B22] border border-white/5 rounded-2xl overflow-hidden border-l-2 ${verdict.border} transition-all duration-200`}>
      ...
      <div className={`bg-gradient-to-r ${verdict.headerBg} px-5 pt-4 pb-3`}>
        <div className="flex items-center gap-2 mb-2 flex-wrap">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 text-xs font-mono font-bold text-[#FF2D2D] bg-[#FF2D2D]/10 px-2.5 py-1 rounded-md hover:bg-[#FF2D2D]/20 transition-colors border border-[#FF2D2D]/20"
          >
      ...
      {!showClip && (
        <div className="px-5 pb-4 pt-2">
          <button
            onClick={handleWatchPlay}
            className="inline-flex items-center gap-2 text-xs font-semibold text-slate-300 hover:text-[#FF2D2D] transition-colors bg-white/5 hover:bg-[#FF2D2D]/10 px-3 py-2 rounded-lg border border-white/10 hover:border-[#FF2D2D]/30"
          >
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/60 to-transparent px-4 py-3 pointer-events-none">
                <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg ${
                  ...
                  <Zap className={`w-3.5 h-3.5 shrink-0 ${
                    ...
                    annotationData.verdict === 'good_play' ? 'text-[#22C55E]' : 'text-yellow-400'
                  }`} />
          ...
          <div className="flex items-center gap-2 mt-3 flex-wrap">
            ...
            {annotationData && (
              <button
                ...
                className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg border bg-white/5 text-slate-400 border-white/10 hover:text-[#FF2D2D] hover:border-[#FF2D2D]/30 transition-colors disabled:opacity-40"
              >
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-blue-400 uppercase tracking-wider">
                    {highlight.verdict === 'bad' ? 'What They Should Have Done' : 'Why This Worked'}
                  </span>
                </div>
                {showAlternative
