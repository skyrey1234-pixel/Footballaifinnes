...
export default function TacticalTwinLibraryPage() {
  ...
  const reconstructionIds = useMemo(() => data.map((item) => item.id), [data]);
  const stage2Status = trpc.tacticalTwinStage2.libraryStatus.useQuery(
    { reconstructionIds },
    { enabled: reconstructionIds.length > 0 },
  );
  const statusByTwin = useMemo(() => new Map((stage2Status.data ?? []).map((row) => [row.reconstructionId, row])), [stage2Status.data]);
    ...
    <div className="space-y-6">
      <header className="relative overflow-hidden border border-emerald-400/20 bg-[#050a08] px-6 py-8">
        ...
        <div className="relative max-w-3xl">
          <p className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.24em] text-emerald-400"><Sparkles className="h-3.5 w-3.5" />Tactical Twin · Stage 2 Vision Lab</p>
          ...
          <p className="mt-4 max-w-2xl text-sm leading-7 text-white/50">Select a real play from Film Breakdown, Highlight Reel, or Live View. Automatically track players and the ball, correct uncertain reads, step frame by frame, study the synchronized map and 3D Twin, then package the moment as a shareable cinematic replay.</p>
