...
import { Film, AlertTriangle, CheckCircle2, Zap, Shield, Crosshair, Star, Filter } from 'lucide-react';
import AnnotatedHighlightCard from './AnnotatedHighlightCard';

const FILTERS = [
  { id: 'all', label: 'All Plays', icon: Film },
  { id: 'mistake', label: 'Mistakes', icon: AlertTriangle },
  { id: 'good', label: 'Good Plays', icon: CheckCircle2 },
...
export default function FilmBreakdown({ session, report }) {
  ...
  const offenseCount = highlights.filter((h) => h.category === 'offense').length;
  const defenseCount = highlights.filter((h) => h.category === 'defense').length;

  if (!highlights.length) {
    return (
      <div className="text-center py-16 border border-dashed border-white/10 rounded-2xl">
        <Film className="w-10 h-10 text-slate-600 mx-auto mb-3" />
        <p className="text-sm text-slate-500">No highlights available for film breakdown.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Click any play to watch with AI circles, arrows, and coaching overlays
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: 'Total Plays', value: highlights.length, color: 'text-white', bg: 'bg-white/5' },
          { label: 'Mistakes', value: mistakeCount, color: 'text-red-400', bg: 'bg-red-500/5' },
          { label: 'Good Plays', value: goodCount, color: 'text-[#00FF87]', bg: 'bg-[#00FF87]/5' },
          { label: 'Offense Plays', value: offenseCount, color: 'text-blue-400', bg: 'bg-blue-500/5' },
        ].map((stat) => (
        ...
        {FILTERS.map((f) => {
          ...
          return (
            ...
            >
              ...
              {f.label}
              {f.id !== 'all' && (
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${isActive ? 'bg-[#00FF87]/20' : 'bg-white/10'}`}>
                  {f.id === 'mistake' ? mistakeCount :
                   f.id === 'good' ? goodCount :
                   f.id === 'offense' ? offenseCount :
                   f.id === 'defense' ? defenseCount :
      ...
      {filteredHighlights.length === 0 ? (
        <div className="text-center py-10 border border-dashed border-white/10 rounded-xl">
          <p className="text-sm text-slate-500">No plays match this filter.</p>
        </div>
      ) : (
      ...
      <div className="flex items-start gap-3 px-4 py-3 bg-purple-500/5 border border-purple-500/15 rounded-xl">
        ...
        <p className="text-xs text-slate-400 leading-relaxed">
          <span className="text-purple-400 font-semibold">Pro tip:</span> Click "Watch with AI Annotations" on any play to see circles, arrows, and coaching overlays drawn directly on the film. Red = mistake, Green = good execution, Blue = suggested movement.
        </p>
      </div>
    </div>
  );
